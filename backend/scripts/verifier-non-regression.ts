/**
 * Vérification de non-régression du contrat d'API après le chantier 1.
 * Compare GET /api/data aux valeurs attendues (baseline _sauvegardes/seed-initial).
 * Les identifiants techniques sont devenus des UUID + `reference` : on compare
 * les champs métier, l'ordre d'affichage et les types sérialisés.
 */
...
async function main() {
  ...
  const { vendors, budgets, purchaseOrders, rfqComparisonPools: rfqs, users, stockItems, stockMovements: movements, assignments } = data as Record<string, any[]>;

  // ── Compteurs ──────────────────────────────────────────────────────
  verif("compteurs", [vendors?.length, budgets?.length, purchaseOrders?.length, rfqs?.length, users?.length, stockItems?.length, movements?.length, assignments?.length].join(",") === "5,4,5,2,5,7,4,3", `${[vendors?.length, budgets?.length, purchaseOrders?.length, rfqs?.length, users?.length, stockItems?.length, movements?.length, assignments?.length].join(",")}`);
  ...
  verif("ordre budgets", budgets.map((b: any) => b.name)[0] === "Technologies de l'Information" && budgets.map((b: any) => b.name)[3] === "Chaîne Logistique & Approvisionnements");
  verif("ordre bons DA-001→DA-005", purchaseOrders.map((p: any) => p.reference).join() === ["DA-2026-001","DA-2026-002","DA-2026-003","DA-2026-004","DA-2026-005"].join());
  verif("ordres rfq-1,rfq-2", rfqs.map((r: any) => r.reference).join() === "rfq-1,rfq-2");
  ...
  const toutesEntites = [...vendors, ...budgets, ...purchaseOrders, ...rfqs, ...users, ...stockItems, ...movements, ...assignments];
  ...
  // ── Décimaux → nombres ─────────────────────────────────────────────
  verif("montants numériques (PO)", purchaseOrders.every((p: any) => typeof p.amount === "number"));
  ...
  verif("createdDate yyyy-MM-dd", purchaseOrders.every((p: any) => estDateSeule(p.createdDate)));
  verif("deliveryDate yyyy-MM-dd", purchaseOrders.every((p: any) => estDateSeule(p.deliveryDate)));
  ...
  verif("v-1 Apex / spend 78500 / Preferred", v1.name === "Apex Tech & Logistique Maroc" && v1.totalSpend === 78500 && v1.status === "Preferred");
  const da1 = purchaseOrders.find((p: any) => p.reference === "DA-2026-001");
  verif("DA-001 montant 45000 / Approved", da1.amount === 45000 && da1.status === "Approved" && da1.items.length === 2);
  const stk1 = stockItems.find((s: any) => s.reference === "STK-001");
  verif("STK-001 qty 15/9/6, prix 14500", stk1.quantity === 15 && stk1.availableQty === 9 && stk1.allocatedQty === 6 && stk1.unitPriceMAD === 14500);
  ...
  for (const route of ["/api/pos", "/api/rfq", "/api/ai/analyze-bids"]) {
    ...
    let body: any = {};
    try { body = JSON.parse(texte); } catch { /* réponse non JSON = échec du contrôle */ }
    verif(`route gelée POST ${route} → 404`, r.status === 404 && typeof body.error === "string" && body.error.includes("désactivé"), `status=${r.status} body=${texte.slice(0, 80)}`);
  }
