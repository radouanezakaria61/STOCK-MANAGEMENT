...
import { MaterialAssignment, PurchaseOrder } from "../types";
 ...
 */
export function exportPurchaseOrderToPDF(po: PurchaseOrder) {
  const doc = new jsPDF({
    orientation: "portrait",
    unit: "mm",
    format: "a4"
  ...
  // --- HEADER BANNER WITH DISTRA LOGO ---
  try {
    const logoUri = getDistraLogoDataUri();
    if (logoUri) {
      doc.addImage(logoUri, "PNG", margin, y, 42, 16);
    }
  } catch (err) {
    doc.setTextColor(132, 189, 0);
    doc.setFont("helvetica", "bold");
  ...
  const bannerX = margin + 46;
  const bannerW = contentWidth - 46;
  doc.setFillColor(30, 41, 59); // Slate-800
  doc.roundedRect(bannerX, y, bannerW, 18, 1.5, 1.5, "F");

  doc.setTextColor(255, 255, 255);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(11);
  doc.text("BON DE COMMANDE & ENGAGEMENT D'ACHAT (DA)", bannerX + 4, y + 6.5);

  doc.setFontSize(7.5);
  doc.setFont("helvetica", "normal");
  doc.setTextColor(203, 213, 225);
  doc.text(`COMMANDE N° ${po.id} • DÉPARTEMENT ACHATS & FINANCE`, bannerX + 4, y + 12.5);
  ...
  doc.setFontSize(8.5);
  doc.setTextColor(100, 116, 139);
  doc.text(`Casablanca, Maroc — Date d'émission : ${po.createdDate}`, margin, y);

  // Status Badge
  const statusColor = po.status === "Approved" || po.status === "Fulfilled" ? [16, 185, 129] : po.status === "Pending Approval" ? [245, 158, 11] : [239, 68, 68];
  doc.setFillColor(statusColor[0], statusColor[1], statusColor[2]);
  doc.roundedRect(pageWidth - margin - 40, y - 4, 40, 6.5, 1.5, 1.5, "F");
  doc.setTextColor(255, 255, 255);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(7.5);
  const statusLabel = po.status === "Approved" ? "VALIDÉ / APPROUVÉ" : po.status === "Fulfilled" ? "RÉCEPTIONNÉ" : po.status === "Pending Approval" ? "EN ATTENTE VALIDATION" : po.status.toUpperCase();
  doc.text(statusLabel, pageWidth - margin - 20, y + 0.5, { align: "center" });

  y += 7;

  // --- SECTION 1 & 2: 2 CARDS (PO DETAILS & VENDOR) ---
  const colWidth = (contentWidth - 6) / 2;
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(8.5);
  doc.text("1. DÉTAILS DE LA DEMANDE D'ACHAT", margin + 4, y + 5);

  doc.setFontSize(8);
  doc.setTextColor(51, 65, 85);
  let oy = y + 12;
  doc.text("Intitulé DA :", margin + 4, oy);
  doc.setFont("helvetica", "bold");
  doc.text(doc.splitTextToSize(po.title, colWidth - 26)[0] || po.title, margin + 24, oy);

  oy += 6;
  doc.setFont("helvetica", "normal");
  doc.text("Département :", margin + 4, oy);
  doc.setFont("helvetica", "bold");
  doc.text(po.department, margin + 24, oy);

  oy += 6;
  doc.setFont("helvetica", "normal");
  doc.text("Demandeur :", margin + 4, oy);
  doc.text(po.requester, margin + 24, oy);

  oy += 6;
  doc.setFont("helvetica", "normal");
  doc.text("Audit Conformité :", margin + 4, oy);
  doc.setFont("helvetica", "bold");
  doc.setTextColor(16, 185, 129);
  doc.text(`${po.auditScore}/100 (Conforme)`, margin + 28, oy);

  oy += 6;
  doc.setFont("helvetica", "normal");
  doc.setTextColor(51, 65, 85);
  doc.text("Devise Utilisée :", margin + 4, oy);
  doc.setFont("helvetica", "bold");
  doc.text("Dirham Marocain (MAD)", margin + 28, oy);

  // Vendor Box
  const vendorX = margin + colWidth + 6;
  doc.setFillColor(248, 250, 252);
  doc.setDrawColor(226, 232, 240);
  doc.roundedRect(vendorX, y, colWidth, 42, 2, 2, "FD");

  doc.setFillColor(241, 245, 249);
  doc.roundedRect(vendorX, y, colWidth, 7, 2, 2, "F");
  doc.setTextColor(15, 23, 42);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(8.5);
  doc.text("2. FOURNISSEUR ATTRIBUTAIRE", vendorX + 4, y + 5);

  doc.setFontSize(8);
  doc.setTextColor(51, 65, 85);
  let vy = y + 12;
  doc.text("Raison Sociale :", vendorX + 4, vy);
  doc.setFont("helvetica", "bold");
  doc.text(po.vendorName, vendorX + 26, vy);

  vy += 6;
  doc.setFont("helvetica", "normal");
  doc.text("Réf. Fournisseur :", vendorX + 4, vy);
  doc.text(po.vendorId, vendorX + 26, vy);

  vy += 6;
  doc.setFont("helvetica", "normal");
  doc.text("Immatriculation :", vendorX + 4, vy);
  doc.text("Registre de Commerce Casablanca", vendorX + 26, vy);

  vy += 6;
  doc.setFont("helvetica", "normal");
  doc.text("Règlement :", vendorX + 4, vy);
  doc.text("Virement Bancaire à 30 jours", vendorX + 26, vy);

  vy += 6;
  doc.setFont("helvetica", "normal");
  doc.text("Lieu de Livraison :", vendorX + 4, vy);
  doc.setFont("helvetica", "bold");
  doc.text("Magasin Central IT / Casablanca", vendorX + 26, vy);
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(9);
  doc.text("3. DÉCOMPOSITION DES ARTICLES & LIGNES BUDGÉTAIRES", margin, y);
  ...
  doc.text("DÉSIGNATION DE L'ARTICLE / PRESTATION", margin + 12, y + 5);
  doc.text("QTÉ", margin + 115, y + 5);
  doc.text("PRIX UNITAIRE (MAD)", margin + 130, y + 5);
  doc.text("TOTAL LIGNE (MAD)", margin + 160, y + 5);
  ...
  po.items.forEach((item, idx) => {
    ...
    doc.setTextColor(15, 23, 42);
    doc.setFont("helvetica", "bold");
    doc.text(doc.splitTextToSize(item.desc, 98)[0] || item.desc, margin + 12, y + 6);

    doc.setFont("helvetica", "normal");
    doc.text(String(item.qty), margin + 118, y + 6);
    doc.text(`${item.unitPrice.toLocaleString()} MAD`, margin + 130, y + 6);

    doc.setFont("helvetica", "bold");
    doc.setTextColor(30, 41, 59);
    doc.text(`${item.total.toLocaleString()} MAD`, margin + 160, y + 6);
  ...
  y += 2;
  doc.setFillColor(241, 245, 249);
  doc.rect(margin + contentWidth - 80, y, 80, 16, "F");
  doc.setDrawColor(203, 213, 225);
  doc.rect(margin + contentWidth - 80, y, 80, 16, "D");

  doc.setTextColor(71, 85, 105);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(7.5);
  doc.text("TOTAL HT :", margin + contentWidth - 76, y + 5);
  doc.text(`${Math.round(po.amount * 0.833).toLocaleString()} MAD`, margin + contentWidth - 6, y + 5, { align: "right" });

  doc.text("TVA (20%) :", margin + contentWidth - 76, y + 9.5);
  doc.text(`${Math.round(po.amount * 0.167).toLocaleString()} MAD`, margin + contentWidth - 6, y + 9.5, { align: "right" });

  doc.setTextColor(15, 23, 42);
  doc.setFontSize(8.5);
  doc.text("TOTAL ENGAGÉ TTC :", margin + contentWidth - 76, y + 14);
  doc.setTextColor(79, 70, 229);
  doc.text(`${po.amount.toLocaleString()} MAD`, margin + contentWidth - 6, y + 14, { align: "right" });
  ...
  if (po.notes) {
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(7);
    doc.text("CONDITIONS & OBSERVATIONS PARTICULIÈRES :", margin + 3, y + 4.5);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(7.5);
    doc.setTextColor(51, 65, 85);
    doc.text(doc.splitTextToSize(po.notes, contentWidth - 6)[0] || po.notes, margin + 3, y + 9.5);
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(8);
  doc.text("LE RESPONSABLE DEMANDEUR", margin + 4, y + 6);
  doc.setFont("helvetica", "normal");
  doc.setFontSize(7);
  doc.setTextColor(100, 116, 139);
  doc.text(`Émis par : ${po.requester}`, margin + 4, y + 11);
  doc.setTextColor(148, 163, 184);
  doc.text("Date & Signature : ___________________________", margin + 4, y + sigBoxHeight - 4);

  // Direction Achats & Finance
  const dsiSigX = margin + sigBoxWidth + 8;
  doc.setFillColor(255, 255, 255);
  doc.setDrawColor(148, 163, 184);
  doc.roundedRect(dsiSigX, y, sigBoxWidth, sigBoxHeight, 2, 2, "D");
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(8);
  doc.text("DIRECTION DES ACHATS & DIRECTION GÉNÉRALE", dsiSigX + 4, y + 6);
  doc.setFont("helvetica", "normal");
  doc.setFontSize(7);
  doc.setTextColor(100, 116, 139);
  doc.text("Visa Bon à Payer & Engagement Budgétaire", dsiSigX + 4, y + 11);
  doc.setTextColor(148, 163, 184);
  doc.text("Cachet & Signature : ________________________", dsiSigX + 4, y + sigBoxHeight - 4);
  ...
  doc.setTextColor(148, 163, 184);
  doc.text(
    `Bon de Commande Officiel • Dépenses en Dirhams Marocains (MAD) • Système Sourcing & ERP • Page 1/1`,
    pageWidth / 2,
    y + 4,
    { align: "center" }
  );
  ...
  // Save PDF
  const filename = `Bon_de_Commande_${po.id}_${po.vendorName.replace(/[^a-zA-Z0-9]/g, "_")}.pdf`;
  doc.save(filename);
}
